...
изменения в 0.62
добавлена процедура morepoints, увеличивающая в 16 раз количество опорных точек
по результатам интерполяции - для более свободного обращения в дальнейшем
...
begin
  if ymax>0 then begin
    for i:=0 to Length(Y)-1 do Y[i]:=Y[i]/ymax;
    changed:=true;
  end;
end;
